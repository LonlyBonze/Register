...
new Vue({	
	...
	methods: {
		alertensure:function(){
			...
			this.$http.post('deletedepartscheex',{
				id:this.schetableid
			}).then((res)=>{
				if(res.body){
					this.get_tabledata();
		...
		add_table:function(){
			if(this.updatestatus){
				...
				this.$http.post('updatedepartscheex',{
					id:this.schetableid,
				...
				if(this.selectedID0!=-1&&this.table_name!=''){
					...
					this.$http.post('isthererepeatexname',{
						name:this.table_name}).then((res)=>{
						if(res.body){
							alert("排班模板名称重复，插入失败！");
						}
						else{
							this.$http.post('insertdepartex',{
		...
		showthistable:function(items){
			this.table_editname='[查看]:'+items.departscheexname;
		...
		schelistupdate:function(items){
			...
			this.schetableid=items.departscheexid;
			this.selectedID0=items.departmentid;
			this.selectedValue0=items.departmentname;
			this.table_name=items.departscheexname;
			this.table_note=items.note;
			this.sche_context=items.departsche;
			this.table_editname='[修改]:'+items.departscheexname;
			this.button_text='修改';
			this.showtablesche()
		},
		...
		printpreload:function(items){
			this.table_editname=items.departscheexname;
