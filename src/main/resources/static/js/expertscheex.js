...
new Vue({	
	...
	methods: {
		alertensure:function(){
			...
			this.$http.post('deleteexpertscheex',{
				id:this.schetableid
			}).then((res)=>{
				if(res.body){
					this.get_tabledata();
			...
			if(this.selectedID1!=-1){
				if(searchkey!=''){
					for(var i=0;i<this.tableDataList.length;i++){
						if(this.tableDataList[i].expertid==this.selectedID1&&this.tableDataList[i].expertscheexname.search(searchkey)!=-1){
					...
					for(var i=0;i<this.tableDataList.length;i++){
						if(this.tableDataList[i].expertid==this.selectedID1){
							tmplist.push(this.tableDataList[i])
						}
					}
					this.jump_page=1;
				...
				if(searchkey!=''){
					for(var i=0;i<this.tableDataList.length;i++){
						if(this.tableDataList[i].expertscheexname.search(searchkey)!=-1){
							tmplist.push(this.tableDataList[i])
						}
					}
					this.page_index=1;
		...
		add_table:function(){
			if(this.updatestatus){
				...
				this.$http.post('updateexpertscheex',{
					id:this.schetableid,
				...
				if(this.selectedID0!=-1&&this.table_name!=''){
					...
					this.$http.post('isthererepeatexpertex',{
						name:this.table_name}).then((res)=>{
						if(res.body){
							alert("排班表名称重复，插入失败！");
						}
						else{
							this.$http.post('insertexpertex',{
		...
		showthistable:function(items){
			this.table_editname='[查看]:'+items.expertscheexname;
		...
		get_tabledata:function(){
			this.$http.get('tableImportExpert').then((response)=>{
		...
		schelistupdate:function(items){
			...
			this.schetableid=items.expertscheexid;
			this.selectedID0=items.expertid;
			this.selectedValue0=items.doctorname;
			this.table_name=items.expertscheexname;
			this.table_note=items.note;
			this.sche_context=items.expertsche;
			this.table_editname='[修改]:'+items.expertscheexname;
			this.button_text='修改';
		...
		schelistdelete:function(items){
			...
			this.alert_tip='确定要删除“'+items.expertscheexname+"”吗？";
		},
		...
		printpreload:function(items){
			this.table_editname=items.expertscheexname;
