...
new Vue({	
	...
	methods: {
		alertensure:function(){
			...
			this.$http.post('deletedepartsche',{
				id:this.schetableid
			}).then((res)=>{
				if(res.body){
					this.get_tabledata();
			...
			if(this.selectedID1!=-1){
				if(searchkey!=''){
					for(var i=0;i<this.tableDataList.length;i++){
						if(this.tableDataList[i].departID==this.selectedID1&&this.tableDataList[i].name.search(searchkey)!=-1){
					...
					for(var i=0;i<this.tableDataList.length;i++){
						if(this.tableDataList[i].departID==this.selectedID1){
							tmplist.push(this.tableDataList[i])
						}
					}
					this.jump_page=1;
				...
				if(searchkey!=''){
					for(var i=0;i<this.tableDataList.length;i++){
						if(this.tableDataList[i].name.search(searchkey)!=-1){
							tmplist.push(this.tableDataList[i])
						}
					}
					this.page_index=1;
		...
		add_table:function(){
			if(this.updatestatus){
				...
				this.$http.post('updatedepartsche',{
					id:this.schetableid,
		...
		showthistable:function(items){
			this.table_editname='[查看]:'+items.name;
			this.sche_context=items.sche;
			this.showtablesche()
		},
		...
		get_tabledata:function(){
			this.$http.get('tableDatadepartment').then((response)=>{
		...
		import_page:function(){
			if(this.updatestatus){
				...
				window.location.href="departScheImport"
			}
		},
		...
		schelistupdate:function(items){
			...
			this.schetableid=items.id;
			this.selectedID0=items.departID;
			this.selectedValue0=items.department;
			this.table_name=items.name;
			this.table_note=items.note;
			this.sche_context=items.sche;
			this.table_editname='[修改]:'+items.name;
			this.button_text='修改';
		...
		schelistdelete:function(items){
			...
			this.alert_tip='确定要删除“'+items.name+"”吗？";
		},
		...
		printpreload:function(items){
			this.table_editname=items.name;
